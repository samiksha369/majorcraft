...
;(function ( $, window, document, undefined ) {
	...
	$.fn.wc_variation_form = function() {
		...
		.on( 'click', '.reset_variations', function() {
			...
			// reset all radios by selection first 
			$(".variation input[type=radio]:first").attr('checked', true);
		...
		// On changing an attribute via select 
		...
		// on changing attibut evi aradio 
		.on( 'change', '.pierry-group-swatch-wrapper input:radio', function( event ) {
				
			$( '.variations_form' ).trigger( 'woocommerce_variation_select_change' );
			$( '.variations_form' ).trigger( 'check_variations', [ '', false ] );
			$( this ).blur();
			
			$(this).trigger( 'woocommerce_variation_has_changed' );	
		})
		...
		.on( 'check_variations', function( event, exclude, focus ) {
			...
			// get all atributes via radio
			$(this).find( '.variations input[type=radio]:checked' ).each( function() {
				var attribute_name = $( this ).data( 'attribute_name' ) || $( this ).attr( 'name' );
				current_settings[ attribute_name ] = $( this ).val();
			});
