...
class WooCommerce_Widget_DropdownCart extends WP_Widget {
    ...
    function widget( $args, $instance ) {
		...
		$dipslay_style = $instance['display_style'];
        ...
        <div class="widget_shopping_mini_cart_content " id="<?php echo $this->id ?>-content">
            <?php if ( !$hide_if_empty || $cart_contents_count > 0 ) : ?>
                <div class="dropdown-cart-button <?php echo $hide_if_empty ? 'hide_dropdown_cart_widget_if_empty' : '' ?>" style="<?php echo $hide_if_empty && sizeof( $woocommerce->cart->get_cart() ) == 0 ? "display:none;":"" ?>">
                	...
                	<?php if($dipslay_style == 'drop-down') {?>
	                    ...
	                    <div class="products-in-cart-button"><?php echo $cart_contents_count; ?></div>
	                    <?php }?>
	             </div>
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('display_style') ?>"><?php echo _e('Popup align:', 'woocommerce-ddc') ?></label>
            <select name="<?php echo $this->get_field_name('display_style') ?>" id="<?php echo $this->get_field_id('display_style') ?>" class="widefat">
                <option value="drop-down" <?php selected('drop-down', $instance['display_style']) ?>><?php _e('Dropdown', 'woocommerce-ddc') ?></option>
                <option value="button" <?php selected('button', $instance['display_style']) ?>><?php _e('Button', 'woocommerce-ddc') ?></option>
            </select>
        </p>
            
        <p>
            <label for="<?php echo $this->get_field_id('popup_align') ?>"><?php echo _e('Popup align:', 'woocommerce-ddc') ?></label>
