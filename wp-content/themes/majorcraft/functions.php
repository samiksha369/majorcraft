<?php
require_once 'theme_configuration_additions.php';
...
add_action( 'woocommerce_single_product_summary', 'pierry_buy_now_button', 18);
...
/** mini cart ***/
add_filter( 'widget_title', function($title, $instance = null, $base_id = 'unspecified') {
	if($base_id == 'woocommerce_widget_cart') {
		global $woocommerce;
		$cart_contents_count = $woocommerce->cart->get_cart_contents_count();
		
		return $title.'
			<div class="dropdown-cart-button">
	           <i class="dropdown-total"></i>
	           <div class="products-in-cart-button">'.$cart_contents_count.'</div>
			</div>';
...
if (!is_admin()) {
    function register_my_js() {
        ...
        wp_enqueue_script('header-script', get_stylesheet_directory_uri(). '/assets/js/header.js', array('jquery'), '1.0', true);
        wp_enqueue_script('majorcraft-minicart-script', get_stylesheet_directory_uri(). '/assets/js/minicart.js', array('jquery'), '1.0', true);
...
function pierry_colophon_left_block() {
    echo sprintf('<div class="%1$s">%2$s</div>',
        ...
        '<img class="footer-logo" src="/wp-content/themes/majorcraft/assets/images/footer-logo.png">'
    );
}
...
function pierry_credits_display(){
    // get address and phone for first line in footer
    $address = get_option('pierry_mc_address');
    $phone = get_option('pierry_mc_phone_number');
    if (!empty($address) && !empty($phone)) {
        $first_line = $address . ' | Phone: ' . $phone;
    } elseif (!empty($phone)) {
        $first_line = 'Phone: ' . $phone;
    ...
    //get email and external url for the second line in footer
    $email = get_option('pierry_mc_email');
    $url = get_option('pierry_mc_external_url');
    if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $second_line = "<a href='mailto:$email'>$email</a>";
    }
    if (!empty($url)) {
        $url = '<a target="_blank" href="http://' . $url . '">' . $url . '</a>';
    ...
    $f_menu = wp_nav_menu(array(
        'theme_location'  => 'main',
        'menu_class'      => implode(' ', apply_filters( "tc_sidenav_menu_class",  array('nav', 'sn-nav' ) ) ),
        'walker'          => ! TC_utils::$inst -> tc_has_location_menu('main') ? '' : new TC_nav_walker('main'),
        'echo'            => false,
    ));

    echo sprintf('<div class="%1$s">%2$s<div class="mobile-show">%3$s</div></div>',
        'span6 credits',
        sprintf( '<div class="mobile-hide">%1$s%2$s</div>', $first_line, $second_line),
        $f_menu
    );
}
...
function pierry_colophon_right_block() {
    if (  !apply_filters('tc_show_text_btt', false) ) {
        $button_top = '';
    } else {
    ...
    $link_policy = get_option('pierry_mc_policy_link');
    if (!empty($link_policy)) {
        $policies_link = '<a class="policies-link" href="' .$link_policy .'">Policies</a>';
    } else {
        $policies_link = '';
    }

    echo sprintf('<div class="%1$s">%2$s%3$s%4$s</div>', 'span3 backtop', $policies_link, $button_top, $social_buttons);
}
...
function pierry_buy_now_button() {
    echo '<a href="/checkout/" class="button checkout wc-forward">' . esc_attr( 'Buy Now', 'woocommerce' ) .  '</a>';
}
?>
